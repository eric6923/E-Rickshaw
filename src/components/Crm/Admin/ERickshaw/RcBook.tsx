...
import { Download, Plus, Search, X, FileCheck, ClipboardList } from 'lucide-react';
...
const RCBook = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    console.log('Submitted RC Book:', formData);
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                {rcBooks.length === 0 ? (
                  ...
                  rcBooks.map((rcBook, index) => (
                    <tr key={index}>
                      {/* Render RC book data here */}
                    </tr>
                  ))
                )}
              </tbody>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white dark:bg-gray-800 p-8 rounded-xl w-[600px] max-h-[90vh] overflow-y-auto relative">
              ...
              <div className="flex items-center gap-3 mb-6">
                ...
                <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-100">Add New RC Book</h2>
              </div>
              <form onSubmit={handleSubmit}>
                <div className="grid grid-cols-1 gap-4">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Number Plate Date</label>
                    <input
                      type="date"
                      name="numberPlateDate"
                      value={formData.numberPlateDate}
                      onChange={handleInputChange}
                      className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                    />
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">RC Book Date</label>
                    <input
                      type="date"
                      name="rcBookDate"
                      value={formData.rcBookDate}
                      onChange={handleInputChange}
                      className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
                    />
                  </div>
                  <div className="space-y-4">
                    <label className="flex items-center space-x-3">
                      <input
                        type="checkbox"
                        name="numberPlateGiven"
                        checked={formData.numberPlateGiven}
                        onChange={handleInputChange}
                        className="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                      />
                      <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Number Plate Given</span>
                    </label>
                    <label className="flex items-center space-x-3">
                      <input
                        type="checkbox"
                        name="rcBookGiven"
                        checked={formData.rcBookGiven}
                        onChange={handleInputChange}
                        className="rounded border-gray-300 text-blue-600 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                      />
                      <span className="text-sm font-medium text-gray-700 dark:text-gray-300">RC Book Given</span>
                    </label>
                  </div>
                </div>
